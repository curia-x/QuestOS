...
| PSCI/SMCCC | 通过 SMCCC HVC/SMC wrapper 调用 PSCI `CPU_ON` 启动 secondary CPU | Linux arm64 PSCI/SMCCC 固件调用路径 | 固件接口、CPU bringup 入口、返回码转换 |
...
| 构建系统 | 顶层 Makefile 编排 kernel/app/package，`scripts/Makefile.build` 递归构建 `obj-y`、`built-in.a` 和汇编 offsets | Linux Kbuild 的分层组织方式 | 模块化扩展、结构体 offset 生成、可读编译日志 |
...
idmap 入口。secondary CPU 在 `smp_init()` 路径中建立自己的 idle 上下文，
依次执行 per-CPU、GICv3 和 arch timer 初始化，启动本地调度 timer，最后进入
`run_idle()` 等待调度。
...
- PSCI/SMCCC: 通过 QEMU 提供的固件接口启动 secondary CPU。
...
bringup、调度、syscall 和 EL0 用户程序。后续扩展可以继续沿着现有主线补充
更完整的内存分配、进程生命周期、文件系统和驱动模型。
