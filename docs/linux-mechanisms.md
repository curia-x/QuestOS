...
仓库保留了从 QEMU 固件入口到 EL0 demo 输出的完整路径，便于逐步跟踪
...
| 用户态 package | Python 工具把 demo ELF 和元数据打包进镜像，当前默认包含 `app1` 到 `app10` | 类似 initramfs/initrd 提供早期用户态载体 | 无 VFS 阶段的用户态验证闭环 |
