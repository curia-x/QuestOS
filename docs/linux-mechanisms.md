...
idmap 入口。secondary CPU 建立自己的 idle 上下文，依次执行 per-CPU、GICv3
和 arch timer 初始化，最后进入 `run_idle()` 等待调度。

一次 QEMU `virt` 运行中的串口输出节选如下，部分装载调试输出和重复的用户态
demo 输出已裁剪:
...
secondary CPU bringup、per-CPU 状态和每 CPU round-robin 调度，但还没有实现
IPI、CPU 热拔除、负载均衡或完整 CPU 拓扑发现。
