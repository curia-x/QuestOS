...
#define list_first_entry_or_null(ptr, type, member) ({ \
	...
	struct list_head *pos__ = (*(const volatile typeof(head__->next) *)&(head__->next)); \
...
{
	return !(*(const volatile typeof(h->pprev) *)&(h->pprev));
}
...
{
	return !(*(const volatile typeof(h->first) *)&(h->first));
}
...
static inline void __hlist_del(struct hlist_node *n)
{
	...
	*(volatile typeof(*pprev) *)&(*pprev) = (next);
	if (next)
		*(volatile typeof(next->pprev) *)&(next->pprev) = (pprev);
}
...
static inline void hlist_add_head(struct hlist_node *n, struct hlist_head *h)
{
	...
	*(volatile typeof(n->next) *)&(n->next) = (first);
	if (first)
		*(volatile typeof(first->pprev) *)&(first->pprev) = (&n->next);
...
{
	*(volatile typeof(n->pprev) *)&(n->pprev) = (next->pprev);
	*(volatile typeof(n->next) *)&(n->next) = (next);
	*(volatile typeof(next->pprev) *)&(next->pprev) = (&n->next);
				    ...
				    struct hlist_node *prev)
{
	...
	*(volatile typeof(n->pprev) *)&(n->pprev) = (&prev->next);

	if (n->next)
		*(volatile typeof(n->next->pprev) *)&(n->next->pprev) = (&n->next);
}
