...
#ifdef CONFIG_ARM64
/* 为解决 warning: call-clobbered register used for global register variable 问题
 * 将x18改成x19，x18在arm64中是平台寄存器，不能被编译器当作全局寄存器变量使用。
 */
 ...
 * 将x18改成x19，x18在arm64中是平台寄存器，不能被编译器当作全局寄存器变量使用。
 */
#define DECLARE_GLOBAL_DATA_PTR		register gd_t *gd asm ("x19")
#else
