...
static int cpuhp_per_cpu_setup(unsigned int cpu, void *data)
{
	...
	if (cpu >= MAX_CPUS) {
		printf("Invalid CPU number %u for percpu setup\n", cpu);
		return -EINVAL;
	}
...
int per_cpu_init(void)
{
	...
	if (err) {
		printf("Failed to register percpu setup callback: %d\n", err);
		return err;
	}
