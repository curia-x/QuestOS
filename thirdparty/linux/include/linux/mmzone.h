...
// 这两项是给 “未被接受的内存（unaccepted memory）” 用的，
// 只有在打开 CONFIG_UNACCEPTED_MEMORY 时才存在
//（通常出现在机密计算/虚拟化来宾场景，比如 SEV-SNP、TDX：
// 来宾拿到的物理内存需要先执行“accept”流程，才能安全地读写）。
#ifdef CONFIG_UNACCEPTED_MEMORY
...
// “动态水位线加成”（单位：pages）：把 min/low/high/promo 这些正常水位线临时抬高，
//                              用来 加大回收压力、提前唤醒 kswapd、缓解碎片化导致的分配回退。
...
enum {
	ZONELIST_FALLBACK,	/* zonelist with fallback */ // 可以退回到其他node的zone中分配
#ifdef CONFIG_NUMA
	/*
	 * The NUMA zonelists are doubled because we need zonelists that
	 * restrict the allocations to a single node for __GFP_THISNODE.
	 */
	ZONELIST_NOFALLBACK,	/* zonelist without fallback (__GFP_THISNODE) */ // 只在自己当前的node中分配
#endif
	MAX_ZONELISTS
};
